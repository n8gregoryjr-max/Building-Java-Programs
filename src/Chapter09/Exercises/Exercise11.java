...
FilteredAccount. A cash processing company has a class called Account used to process transactions:
    ...
    public boolean process(Transaction t)	Processes the next transaction, returning true if the
                                                transaction was approved and false otherwise
...
More specifically, the new class should indicate that a zero-valued transaction was approved but shouldn’t call the process method for it.
Your class should have a single constructor that accepts a parameter of type Client, and it should include the following method:
